import { wrapHookToRecognizeElement, useCollector, RenderIndicator, getDOMInfo, deprecationWarning, useEffectOnce, ERROR_TOP_LEVEL_ELEMENT_NO_ID, ROOT_NODE, ERROR_DELETE_TOP_LEVEL_NODE, ERROR_INVALID_NODEID, ERROR_NOPARENT, DEPRECATED_ROOT_NODE, ERROR_MOVE_TOP_LEVEL_NODE, ERROR_MOVE_NONCANVAS_CHILD, ERROR_CANNOT_DRAG, ERROR_MOVE_TO_NONCANVAS_PARENT, ERROR_MOVE_INCOMING_PARENT, ERROR_MOVE_CANNOT_DROP, ERROR_MOVE_TO_DESCENDANT, ERROR_DUPLICATE_NODEID, ERROR_MOVE_OUTGOING_PARENT, ERROR_INVALID_NODE_ID, ERROR_DESERIALIZE_COMPONENT_NOT_IN_RESOLVER, ERROR_NOT_IN_RESOLVER, useMethods, ERROR_RESOLVER_NOT_AN_OBJECT } from '@craftjs/utils';
export { ROOT_NODE } from '@craftjs/utils';
import React, { createContext, useContext, useMemo, useRef, useEffect, useState, Children, Fragment } from 'react';
...
var useEventHandler = function () { return useContext(EventHandlerContext); };
...
function useInternalEditor(collector) {
    ...
    var connectors = useMemo(function () { return handlers && handlers.connectors(); }, [
        handlers,
    ]);
...
var Events = function (_a) {
    ...
    return handlers ? (React.createElement(EventHandlerContext.Provider, { value: handlers },
        indicator &&
            React.createElement(RenderIndicator, {
                style: _assign(_assign({}, movePlaceholder(indicator.placement, getDOMInfo(indicator.placement.parent.dom), indicator.placement.currentNode &&
                    getDOMInfo(indicator.placement.currentNode.dom))), { backgroundColor: indicator.error
...
var NodeProvider = function (_a) {
    ...
    return (React.createElement(NodeContext.Provider, { value: { id: id, related: related, connectors: connectors } }, children));
};
...
var Render = function () {
    ...
    return useMemo(function () {
        ...
        if (nodes && nodes.length > 0) {
            children = (React.createElement(React.Fragment, null, nodes.map(function (id) { return (React.createElement(NodeElement, { id: id, key: id })); })));
        }
        var render = React.createElement(type, props, children);
...
var RenderNodeToElement = function () {
    ...
    return React.createElement(onRender, { render: React.createElement(Render, null) });
};

var NodeElement = React.memo(function (_a) {
    var id = _a.id;
    return (React.createElement(NodeProvider, { id: id },
        React.createElement(RenderNodeToElement, null)));
});
...
function Element(_a) {
    ...
    var _e = useState(null), linkedNodeId = _e[0], setLinkedNodeId = _e[1];
    useEffectOnce(function () {
        ...
        if (inNodeContext) {
            ...
            if (existingNode && existingNode.data.type === is) {
                ...
                var linkedElement = React.createElement(Element, elementProps, children);
...
var deprecateCanvasComponent = function () {
    return deprecationWarning('<Canvas />', {
        suggest: '<Element canvas={true} />',
    });
};
function Canvas(_a) {
    var props = __rest(_a, []);
    useEffect(function () { return deprecateCanvasComponent(); }, []);
    return React.createElement(Element, _assign({}, props, { canvas: true }));
}
...
var Frame = function (_a) {
    ...
    var _c = useState(null), render = _c[0], setRender = _c[1];
    if (!!json) {
    ...
    useEffect(function () {
        ...
        if (initialData) {
            ...
            var rootNode_1 = React.Children.only(initialChildren);
...
function connectEditor(collect) {
    return function (WrappedComponent) {
        return function (props) {
            ...
            return React.createElement(WrappedComponent, _assign({}, Editor, props));
        };
    };
}
...
function connectNode(collect) {
    return function (WrappedComponent) {
        return function (props) {
            ...
            return React.createElement(WrappedComponent, _assign({}, node, props));
        };
    };
}
...
var serializeComp = function (data, resolver) {
    ...
    props = Object.keys(props).reduce(function (result, key) {
        ...
        if (key === 'children' && typeof prop !== 'string') {
            result[key] = Children.map(prop, function (child) {
    ...
    return {
        ...
        isDraggable: function (onError) {
            try {
                ...
                invariant(NodeHelpers(state, targetNode.data.parent).isCanvas(), ERROR_MOVE_NONCANVAS_CHILD);
                invariant(targetNode.rules.canDrag(targetNode, nodeHelpers), ERROR_CANNOT_DRAG);
                return true;
            }
            catch (err) {
                if (onError) {
        ...
        isDroppable: function (target, onError) {
            ...
            try {
                ...
                if (typeof target === 'string') {
                    invariant(!nodeHelpers(target).isTopLevelNode(), ERROR_MOVE_TOP_LEVEL_NODE);
                }
                invariant(this.isCanvas(), ERROR_MOVE_TO_NONCANVAS_PARENT);
                invariant(newParentNode.rules.canMoveIn(targetNode, newParentNode, nodeHelpers), ERROR_MOVE_INCOMING_PARENT);
                invariant(targetNode.rules.canDrop(newParentNode, targetNode, nodeHelpers), ERROR_MOVE_CANNOT_DROP);
                if (isNewNode) {
                    return true;
                }
                var targetDeepNodes = nodeHelpers(targetNode.id).descendants(true);
                invariant(!targetDeepNodes.includes(newParentNode.id) &&
                    newParentNode.id !== targetNode.id, ERROR_MOVE_TO_DESCENDANT);
                ...
                invariant(currentParentNode ||
                    (!currentParentNode && !state.nodes[targetNode.id]), ERROR_DUPLICATE_NODEID);
                ...
                if (node !== currentParentNode) {
                    invariant(currentParentNode.rules.canMoveOut(targetNode, currentParentNode, nodeHelpers), ERROR_MOVE_OUTGOING_PARENT);
                }
                return true;
            }
            catch (err) {
            ...
            if (actualType.craft.related) {
                ...
                Object.keys(actualType.craft.related).forEach(function (comp) {
                    node.related[comp] = function () {
                        return React.createElement(NodeProvider, relatedNodeContext_1, React.createElement(actualType.craft.related[comp]));
                    };
                });
            }
        }
...
var deserializeComp = function (data, resolver, index) {
    ...
    var jsx = _assign({}, React.createElement(main, _assign({}, props)));
...
var deserializeNode = function (data, resolver) {
    ...
    invariant(isCompAnHtmlElement || isCompAUserComponent, ERROR_DESERIALIZE_COMPONENT_NOT_IN_RESOLVER.replace('%displayName%', data.displayName).replace('%availableComponents%', Object.keys(resolver).join(', ')));
        ...
        getDropPlaceholder: function (source, target, pos, nodesToDOM) {
            ...
            var dimensionsInContainer = targetParentNodes
                ? targetParentNodes.reduce(function (result, id) {
                    ...
                    if (dom) {
                        var info = _assign({ id: id }, getDOMInfo(dom));
                        result.push(info);
                    }
                    return result;
                }, [])
        ...
        parseReactElement: function (reactElement) { return ({
            toNodeTree: function (normalize) {
                ...
                if (reactElement.props && reactElement.props.children) {
                    childrenNodes = React.Children.toArray(reactElement.props.children).reduce(function (accum, child) {
...
var useEditorStore = function (options) {
    return useMethods(ActionMethodsWithConfig, _assign(_assign({}, editorInitialState), { options: _assign(_assign({}, editorInitialState.options), options) }), QueryMethods);
};
...
var Editor = function (_a) {
    ...
    if (options.resolver !== undefined) {
        invariant(typeof options.resolver === 'object' && !Array.isArray(options.resolver), ERROR_RESOLVER_NOT_AN_OBJECT);
    }
    ...
    return context ? (React.createElement(EditorContext.Provider, { value: context },
        React.createElement(Events, null, children))) : null;
};

export { ActionMethodsWithConfig, Canvas, CoreEventHandlers, DefaultEventHandlers, DerivedEventHandlers, Editor, Element, Events, Frame, Handlers, NodeContext, NodeProvider, connectEditor, connectNode, defaultElementProps, defineEventListener, deprecateCanvasComponent, editorInitialState, elementPropToNodeData, useEditor, useEditorStore, useEventHandler, useNode };
